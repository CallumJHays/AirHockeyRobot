...
class Arm(SimObject):
    ...
    def draw(self):
        ...
        #print(self.globalBaseAngle)
        if self.globalBaseAngle > 0 and self.globalBaseAngle < 90:
            self.lowerArm.set(self.globalBaseAngle)

        """Gets angle to the end arm"""
        self.globalMidArmAngle = self.getMidArmAngle(target)
        a = self.calculateInverseKinematics(target)[2]
        print(360-(a%360))
        self.upperArm.set(360-(a%360)-self.upperArm.minDeg)
